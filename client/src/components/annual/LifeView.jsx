...
function LifeCatIcon({ catKey, lb, className, style }) {
  ...
  const isPlant = catKey === 'planting' || lb === '种植';
  const known = ['relation', 'food', 'travel', 'movie', 'shop', 'birthday', 'planting'];
  // Flower2 是 lucide-react 独立组件，单独 early-return 避免嵌套 svg stroke 干扰
  if (isPlant) return <Flower2 className={cls} style={style} strokeWidth={2} />;
  return (
    <svg className={cls} style={style} fill="none" stroke="currentColor" strokeWidth="2"
      ...
      {!isPet && !isPlant && !known.includes(catKey) && (<><path d="M12.586 2.586A2 2 0 0 0 11.172 2H4a2 2 0 0 0-2 2v7.172a2 2 0 0 0 .586 1.414l8.704 8.704a2.426 2.426 0 0 0 3.42 0l6.58-6.58a2.426 2.426 0 0 0 0-3.42z"/><circle cx="7.5" cy="7.5" r="1" fill="currentColor" stroke="none"/></>)}
    </svg>
  );
}
