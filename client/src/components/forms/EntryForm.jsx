...
const BTN_PRIMARY = { padding: '6px 14px', borderRadius: '9px', fontSize: '13px', fontWeight: '600', background: '#f9a8a8', color: '#fff', border: 'none', cursor: 'pointer', transition: 'all .15s', boxShadow: '0 2px 8px rgba(249,168,168,0.25)' };
...
export default function EntryForm({ initial, categoryLabel, onSaved, onCancel, onDelete }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
      {categoryLabel && (
        <div style={{ fontSize: '12px', fontWeight: '600', color: '#8e8e93', textTransform: 'uppercase', letterSpacing: '0.03em' }}>
          {categoryLabel}
        </div>
      )}
      <div>
