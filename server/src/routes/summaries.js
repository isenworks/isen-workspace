...
// === D1 动作式路由别名 ===
// D1 格式：POST body 带 {date} 或 {from,to}
router.post('/get', (req, res) => {
  const { date, from, to } = req.body || {};
  if (from && to) {
    const row = db.prepare(`SELECT * FROM summaries WHERE user_id = ? AND date >= ? AND date <= ? ORDER BY date DESC`).all(req.user.id, from, to);
    return res.json({ summaries: row });
  }
  if (date) {
    const row = db.prepare(`SELECT * FROM summaries WHERE user_id = ? AND date = ?`).get(req.user.id, date);
    return res.json({ summary: row });
  }
  res.status(400).json({ error: '请提供 date 或 from/to' });
});
